...
!#endif
	...
	stat_info     = 0
        ...
        !------ Added by Adolfo for Emanuele -----
        !-- Mark as true if you want the velocity gradient tensor 
        !   to be written out --
        write_vgt = .TRUE.
        IF (write_vgt) THEN
           NULLIFY(vgt)
        ...
        !----- Added by Adolfo for Emanuele -----
        !-- Mark as true if you want the transport coefficients
        !   to be written out --
        write_tr_coeff = .TRUE.
        IF (write_tr_coeff) THEN
           NULLIFY(a)
           NULLIFY(b)
        ENDIF
        !----------------------------------------
...
!#endif
	...
	!----------------------------------------------------
        ...
        !--- Added by Adolfo for Emanuele ---
        IF (write_vgt) THEN
           CALL particles_get_vgt(d_particles,vgt,num_part,stat_info)
        ENDIF
        IF (write_tr_coeff) THEN
           CALL particles_get_a(d_particles,a,num_part,stat_info)
           CALL particles_get_b(d_particles,b,num_part,stat_info)
        ENDIF           
        !------------------------------------
...
!#endif
        ...
        IF (write_vgt) THEN
           DO I = 1, num_part
              output(current_dim+1:current_dim+num_vgt,I) = vgt(1:num_vgt,I)
           ENDDO
           current_dim = current_dim + num_vgt
        ENDIF
        IF (write_tr_coeff) THEN
           DO I = 1, num_part
              output(current_dim+1,I) = a(I)
              output(current_dim+2,I) = b(I)
           ENDDO
           current_dim = current_dim + 2
        ENDIF
        !--------------------------------------
